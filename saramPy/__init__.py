...
class Saram(object):
            ...
            for i, line in enumerate(f):
                ...
                if i == line_number - 2:
                    ...
                    if comment is not None:
                        self.comment = comment
                    self.command_run = (
                        "Script dump" if script_name is None else script_name
                    )
                    self.output = "".join(lines)
                    return self

    def file_content(
        self, file_path: str, comment: str = None, file_name: str = None
    ) -> Saram:
        """
    ...
    def variable_output(
        self, var: any, comment: str = None, script_name: str = None
    ) -> Saram:
        """
...
class SaramInit:
    ...
    def _verify_api_key(self):
        r = requests.post(f"{self.saram_url}misc/valid/key", json={"key": self.api_key})
        if r.status_code == 200:
            return r.json()
        else:
