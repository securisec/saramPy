...
def main():
    ...
    parse.add_argument('--init', dest='init',
                       help='Configure Saram config file. Pass the api key')
    ...
    if options.init:
        i = SaramInit(options.init, local=options.local,
    ...
    # main argument parser
        ...
        group.add_argument('-c', '--command', dest='command',
                           nargs=argparse.REMAINDER, help='Command to run inside quotes')
        ...
        if args.command:
            ...
            p.file_content(args.file, file_name=Path(
                args.file).parts[-1], comment=args.comment).send_to_server()
